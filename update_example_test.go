...
func ExampleObject_Del() {
	v := MustParse(`{"foo": 123, "bar": [1,2], "baz": "xyz"}`)
	o, err := v.Object()
	if err != nil {
...
func ExampleValue_Set() {
	v := MustParse(`{"foo":1,"bar":[2,3]}`)

	// Replace `foo` value with "xyz"
	v.Set("foo", MustParse(`"xyz"`))
	// Add "newv":123
	v.Set("newv", MustParse(`123`))
	fmt.Printf("%s\n", v)

	// Replace `bar.1` with {"x":"y"}
	v.Get("bar").Set("1", MustParse(`{"x":"y"}`))
	// Add `bar.3="qwe"
	v.Get("bar").Set("3", MustParse(`"qwe"`))
	fmt.Printf("%s\n", v)
